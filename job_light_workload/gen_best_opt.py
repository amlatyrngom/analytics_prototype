...
def gen_best_mat_views(size_budget):
    ...
    while len(curr_mat_df) > 0:
        ...
        tmp_df = curr_mat_df[curr_mat_df.extra_size <= curr_available_budget].groupby(by=['opt_name', 'extra_size']).agg({"cost_savings": "sum"})
        tmp_df.columns = ["cost_savings"]
        tmp_df = tmp_df.reset_index()
        tmp_df['cost_savings_over_size'] = tmp_df.cost_savings / tmp_df.extra_size
        tmp_df = tmp_df.sort_values(by='cost_savings_over_size', ascending=False).reset_index(drop=True)[:1]
