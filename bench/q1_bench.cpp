...
#include "execution/plan_nodes/plan_node.h"
#include "execution/plan_nodes/hash_join_node.h"
#include "execution/plan_nodes/hash_aggr_node.h"
#include "execution/plan_nodes/static_aggregation.h"
#include "execution/plan_nodes/sort_node.h"
#include "storage/table_loader.h"
using namespace smartid;

class Q1Fixture : public benchmark::Fixture {
 public:
  void SetUp(const benchmark::State& state) override {
    std::cout << "LOADTPCH" << std::endl;
    TableLoader::LoadTPCH("tpch_data");
  }
};

// Q1
BENCHMARK_DEFINE_F(Q1Fixture, Q1)(benchmark::State& st) {
    ...
    for (auto _ : st) {
      ...
      ScanNode scan("lineitem", std::move(projections), std::move(filters));
      // Aggregation
...
BENCHMARK_DEFINE_F(Q1Fixture, Q4)(benchmark::State& st) {
  for (auto _ : st) {
    // Orders scan
    // Cols
    ...
    hash_join_node.UseBloomFilter(60000, 0.1);
    // Agg
...
// Register
//BENCHMARK_REGISTER_F(Q1Fixture, Q1)->DenseRange(0, 5);
BENCHMARK_REGISTER_F(Q1Fixture, Q4)->DenseRange(0, 5);
