...
public:
  ...
  inline void smart_insert(int64_t hash)
  {
    std::size_t bit_index = 0;
    std::size_t bit       = 0;
    ...
    for (std::size_t i = 0; i < salt_.size(); ++i)
    {
      auto h = static_cast<int>(hash) + static_cast<int>(hash >> 32ll);
      compute_indices(h, bit_index, bit);
  ...
  inline virtual bool smart_contains(int64_t hash) const
  {
    std::size_t bit_index = 0;
    std::size_t bit       = 0;
