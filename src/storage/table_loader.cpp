...
// */
//std::pair<Table *, std::string> CreateTest1(Catalog* catalog, const std::string &directory, const std::string &table_name) {
...
//  auto table = catalog->CreateTable(table_name, &schema);
...
// */
//std::pair<Table *, std::string> CreateTest2(Catalog* catalog, const std::string &directory, const std::string &table_name) {
//  std::string filename = directory + "/" + table_name + ".tbl";
//  std::vector<Column> cols = {
//      Column::ScalarColumn("col1", SqlType::Int64),
//      Column::ScalarColumn("col2", SqlType::Int32),
...
// */
//std::pair<Table *, std::string> CreateTest3(Catalog* catalog, const std::string &directory, const std::string &table_name) {
//  std::string filename = directory + "/" + table_name + ".tbl";
//  std::vector<Column> cols = {
//      Column::ScalarColumn("col1", SqlType::Int64),
//      Column::ScalarColumn("col2", SqlType::Int32),
...
//          auto table_info = CreateFn(catalog, directory, name); \
//          LoadTable(table_info.first, table_info.second); \
//        }
//
//void TableLoader::LoadTestTables(Catalog* catalog, const std::string &directory) {
//
...
//std::unique_ptr<WorkloadInfo> ReadWorkloadInfo(toml::table& tml) {
...
//      cols.emplace_back(Column(std::string(col_name), col_type, is_pk, is_fk, fkname));
//    }
//    Schema schema(std::move(cols));
...
//    res->allocated_filters.emplace_back(std::make_unique<LogicalFilter>());
//    auto filter = res->allocated_filters.back().get();
...
//    }
//    scan->filters.emplace_back(filter);
//  }
//  std::cout << "ReadScan: ";
...
//}
//
//void TableLoader::LoadWorkload(Catalog *catalog, const std::string &toml_file) {
//  // Open toml file.
//  toml::table tml;
//  try {
...
//    throw err;
//  }
//  auto workload_info = ReadWorkloadInfo(tml);
//  if (!workload_info->reload) {
//    return;
//  }
//  ReadQueries(tml, workload_info.get());
//  EnumerateAllJoinOrders(workload_info.get());
//  LoadData(catalog, workload_info.get());
//  SampleTables(catalog, workload_info.get());
//}
