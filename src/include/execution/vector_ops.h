...
struct VectorOps {
   ...
   */
  static void GatherVector(const Vector *in, const Bitmap *filter, SqlType in_type, uint64_t val_offset, Vector *out);

  /**
   * Scatter elements from the input vector the pointers + val_offset within out.
   */
  static void ScatterVector(const Vector *in, const Bitmap *filter, SqlType out_type, uint64_t val_offset, Vector *out);

  /**
   * Test and set mark value at given index.
   */
  static void TestSetMarkVector(Bitmap *filter, uint64_t val_offset, Vector *out);
   ...
   */
  static void HashVector(const Vector *in, const Bitmap *filter, Vector *out);

  /**
   * Hash values from the input vector combined with the output vector.
   */
  static void HashCombineVector(const Vector *in, const Bitmap *filter, Vector *out);
...
//                                Vector *out);
   ...
   */
  static void ReduceVector(const Bitmap *filter, const Vector *in, Vector *out, AggType agg_type);

  /**
   * Reduce value from the input vector into the output value after scattering them to the right offset.
   */
  static void ScatterReduceVector(const Bitmap *filter,
                                  const Vector *in,
                                  Vector *out,
                                  uint64_t agg_offset,
                                  AggType agg_type);
   ...
   */
  static void ScatterInitReduceScalar(const Vector *in, uint64_t i, char *out, uint64_t agg_offset, AggType agg_type);

  /**
   * Scatter input element at the given index into the output array at the given offset.
   */
  static void ScatterScalar(const Vector *in, uint64_t i, char *out, uint64_t val_offset);
...
//
  static void HeapInsertVector(const Bitmap* filter, uint64_t limit, char** entries, std::vector<char*>& sort_vector,
                               const std::vector<std::pair<uint64_t, SortType>> &sort_keys,
                               const std::vector<SqlType> &val_types,
                               const std::vector<uint64_t> &val_offsets);
...
//
//  static void CopyVector(const Bitmap* filter, const Vector* in, char* out);
   ...
   * @param out_idx
   */
  static void ReadBlock(const RawTableBlock* block, const std::vector<std::pair<uint64_t, uint64_t>>& row_idxs, uint64_t col_idx, Vector* out);
};
}