...
class ScalarHashJoinExecutor: public PlanExecutor {
 public:
  ScalarHashJoinExecutor(HashJoinNode* node, std::vector<std::unique_ptr<PlanExecutor>> && children)
 ...
 private:
  ...
  // Join hash table.
  bool is32{false};
  JoinTable64 join_table64_;
  JoinTable32 join_table32_;
  // Stores hash values.
